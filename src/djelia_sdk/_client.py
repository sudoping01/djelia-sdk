...
class DjeliaSDK(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous DjeliaSDK client instance.

        This automatically infers the `api_key` argument from the `DJELIA_SDK_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("DJELIA_SDK_API_KEY")
        if api_key is None:
            raise DjeliaSDKError(
                "The api_key client option must be set either by passing api_key to the client or by setting the DJELIA_SDK_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("DJELIA_SDK_BASE_URL")
        if base_url is None:
            base_url = f"https://api.example.com"

        custom_headers_env = os.environ.get("DJELIA_SDK_CUSTOM_HEADERS")
...
class AsyncDjeliaSDK(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncDjeliaSDK client instance.
